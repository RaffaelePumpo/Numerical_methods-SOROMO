function [F0,Q_a,J] = TIDM_spectral(qn_0,rn_0,q_0,r_0,eta_0,eta_dot_0,a,b,time,Const,Config, q, dot_q, ddot_q)
...
% X_grille=Const.L*(-x+1)/2; % sur une grille [0;L]
% DX =(-2/Const.L)*D_Q;      % Matrice de différentiation sur une grille [0;L]

[DX,X_grille]=cheb(N_noeuds-1,Const.L);  % sur une grille [0,L]
...
%     Delta_eta_dot_0 = [[b*T_SO3_m1_(Theta),zeros(3)];[hat_(eta_dot_0(4:6)+hat_(eta_0(1:3))*eta_0(4:6))+a*hat_(eta_0(4:6))*T_SO3_m1_(Theta)-hat_(eta_0(1:3))*hat_(eta_0(4:6)),b*eye(3)-a*hat_(eta_0(1:3))]]*Delta_psi_0;
    ...
    for it_x = 1:N_noeuds
        ...
        Delta_F_bar(:,it_x)= [zeros(3,1);hat_(Delta_psi_X(:,it_x))'*R'*[0;   0; -Const.rho*Const.Area*Const.Gamma_g]];
    end
